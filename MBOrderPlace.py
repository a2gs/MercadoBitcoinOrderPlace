...
def runOrder(conn, argOrderType : str, argCoin : str, argValue : str) -> bool:
	...
	try:
		...
		print(f"Status: [{response_json['status_code']}]", end = ' ')
		...
		if response_json['status_code'] == 100:
			...
			print("Erro!")
			print(f"Mesage: [{response_json['error_message']}]")
...
if __name__ == '__main__':
	...
	while True:
		...
		if argCompare == 'MAIOR' and price < argReference:
			ret = runOrder(conn, argOrderType, argCoin, argValue)
			break

		elif argCompare == 'MENOR' and price > argReference:
			ret = runOrder(conn, argOrderType, argCoin, argValue)
			break

		elif argCompare == 'IGUAL' and price == argReference:
			ret = runOrder(conn, argOrderType, argCoin, argValue)
			break
		...
		print(f"sleep... Last: {price} | Reference {argReference}")
		time.sleep(2)
